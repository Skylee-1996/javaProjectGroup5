...
public class Baccarat implements IBaccarat {
    ...
    public void start(Scanner scan, String id, int userBalance, UserManager u) throws InterruptedException, IOException {
    	...
    	u.printInBox(id+"님 "+"현재 잔액: " + userBalance + "원");        
        ...
        u.printInBox("뱅커의 점수는 : " +bankerScore + "   플레이어의점수는: "+bm.getValue(playerCards.get(0)));
        ...
        u.printInBox("뱅커의 점수는:"+bankerScore+"  플레이어의 스코어:" + playerScore);
     // 플레이어의 3번째 카드 뽑기
        ...
        // 새로운 점수 표시
        u.printInBox("뱅커의 총 점수는: " + bankerScore+"   플레이어의 총 점수는: " + playerScore);
        String result = bm.getResult(playerScore, bankerScore);
        u.printInBox("결과: " + result + " 승!");
        ...
        if ((result.equals("플레이어") && bet.equals("플레이어")) ||
                ...
                u.printInBox("축하합니다! 배팅 금액이 2배로 반환되었습니다.");
            } else if (result.equals("타이") && bet.equals("타이")) {
                userBalance += betAmount*9; // 타이의 경우, 배팅 금액이 9배로봔환
                u.printInBox("타이입니다! 배팅 금액이 9배로 반환됩니다.");
            }else if(result.equals("타이") && !bet.equals("타이")){
            	u.printInBox("타이입니다! 배팅금액이 그대로 반환됩니다");
            }else {
                userBalance -= betAmount;
                u.printInBox("아쉽네요! 배팅 금액을 잃었습니다.");
            }
        u.printInBox("현재 자본금:" + userBalance + "원");
