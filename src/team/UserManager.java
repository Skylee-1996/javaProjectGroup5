...
public class UserManager {
    ...
    public void deleteUser(Scanner scan) throws IOException {
      ...
      for(int i=0;i<userList.size();i++) {
         if(userList.get(i).getName().equals(id) && userList.get(i).getName().equals(name)) {
       userList.remove(i);
    ...
    public void editUser(Scanner scan) throws IOException {
       ...
       String name = scan.next();
       for(int i=0;i<userList.size();i++) {
        if (userList.get(i).getId().equals(id) && userList.get(i).getName().equals(name)) {
            int choice;
            do {
    ...
    //회원검색 후 출력
      ...
      public void balanceChoice(Scanner scan) {
         ...
         switch(num) {
         case 1: depositMoney(scan); break;
         case 2: withdrawMoney(scan); break;
